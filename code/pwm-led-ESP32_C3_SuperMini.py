...
def sound_connect(buzzer):
    for freq_increment in range(2000, 6000, 1000):
        play_tone(buzzer = buzzer, freq = freq_increment, duration = 150)

def sound_disconnect(buzzer):
    for freq_increment in range(0, 4000, 3000):
        play_tone(buzzer = buzzer, freq = 5000 - freq_increment, duration = 150)
...
buzzer 		= 	machine.PWM(machine.Pin(buzzer_gpio, machine.Pin.OUT))
mosfet 		= 	machine.PWM(machine.Pin(mosfet_gpio, machine.Pin.OUT))
red_led 	= 	machine.PWM(machine.Pin(red_led_gpio, machine.Pin.OUT))
green_led 	= 	machine.PWM(machine.Pin(green_led_gpio, machine.Pin.OUT))
blue_led 	= 	machine.PWM(machine.Pin(blue_led_gpio, machine.Pin.OUT))
